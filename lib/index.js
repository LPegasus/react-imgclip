...
import { loadImage, calcContainerSize, calcOffsetEdge, calcScalePos, getDistance, prefixClipInfo, clipImageByPosition, distanceOfPoints } from './helpers';
import './index.less';
var uuid = 0;
var activeUUID = null;
...
function (_super) {
  ...
  function ImageClip(props, ctx) {
    ...
    _this.internalGetDataURL = function () {
        ...
        return tslib_1.__generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              ...
              return [4
              /*yield*/
              , clipImageByPosition(this.image, clipInfo.x, clipInfo.y, clipInfo.width, clipInfo.height, this.props.imageType || 'jpeg', this.props.quality || 1, this.props.canOverClip ? this.props.fillColor : null, this.props.canOverClip ? this.state.backgroundPositionX * this.scaleRatio : 0, this.props.canOverClip ? this.state.backgroundPositionY * this.scaleRatio : 0)];
    ...
    _this.handleDragStart = function (e) {
      ...
      if (e.touches && e.touches.length === 2) {
        _this.clearDelayTimer(); // 记录初始指间距


        _this.thumbOriginDistance = distanceOfPoints(e.touches[0].clientX, e.touches[0].clientY, e.touches[1].clientX, e.touches[1].clientY);
        _this.moveStartPos = {
          x: NaN,
          y: NaN,
      ...
      var _a = isTouchScreen ? e.touches[0] : e,
          clientX = _a.clientX,
          clientY = _a.clientY;
      ...
      _this.timer = window.setTimeout(function () {
        ...
        _this.timer = NaN; // 计算截图 viewport 可移动范围

        _this.movePosRange = calcOffsetEdge(_this.props.canOverClip ? 0 : _this.state.backgroundPositionX, _this.props.canOverClip ? 0 : _this.state.backgroundPositionY, _this.containerWidth, _this.props.canOverClip ? _this.state.containerHeight : _this.state.backgroundSizeHeight, _this.state.viewportPos.left, _this.state.viewportPos.top, _this.state.viewportPos.width, _this.state.viewportPos.height, 'move');
    ...
    _this.handleDragEnd = function () {
      ...
      _this.setState(function (s) {
        ...
        var _a = s.viewportPos,
            left = _a.left,
            top = _a.top,
            other = tslib_1.__rest(_a, ["left", "top"]);
        ...
        nextState.viewportPos = tslib_1.__assign({
    ...
    _this.handleResizeDragOver = function (e) {
      ...
      if (!_this.state.isResizing) {} else if (e.touches && e.touches.length === 2 && _this.thumbOriginDistance > 0) {
        e.stopPropagation();
        e.preventDefault();
        var delta = distanceOfPoints(e.touches[0].clientX, e.touches[0].clientY, e.touches[1].clientX, e.touches[1].clientY); // 写死 45 度角缩放

        var fixedDelta = 0.707 * (delta - _this.thumbOriginDistance) / 2; // 记录本次距离用于下次比较
        ...
        var width = void 0;
        var height = void 0;
        width = _this.state.viewportPos.width + fixedDelta * 2; // 写死 45 度角缩放
        ...
        var lastRatio = _this.props.ratio || _this.moveStartPos.viewportOriginPos.width / _this.moveStartPos.viewportOriginPos.height;
        ...
        _this.setState(function (s) {
          ...
          var _a = calcScalePos(_this.moveStartPos.viewportOriginPos.left, _this.moveStartPos.viewportOriginPos.top, _this.moveStartPos.viewportOriginPos.width, _this.moveStartPos.viewportOriginPos.height, offset_1.x, offset_1.y, _this.anchorType, _this.props.ratio),
              x = _a.x,
              y = _a.y,
              width = _a.width,
              height = _a.height;
    ...
    _this.fireChange = function () {
      if (typeof _this.props.onChange === 'function') {
        var _a = _this.state.viewportPos,
            left = _a.left,
            top_1 = _a.top,
            height = _a.height,
            width = _a.width;
        var fixedViewPort = prefixClipInfo({
          x: left,
          y: top_1,
          height: height,
          width: width
  ...
  ImageClip.prototype.componentDidMount = function () {
    ...
    var anchors = [this.$leftTopAnchor, this.$leftCenterAnchor, this.$leftBottomAnchor, this.$topCenterAnchor, this.$bottomCenterAnchor, this.$rightBottomAnchor, this.$rightCenterAnchor, this.$rightTopAnchor]; // chrome 56 之后，touch 事件若要使用 preventDefault 禁用滚动、下拉刷新，必须设置 passive: false

    if (isTouchScreen) {
      this.$viewport.addEventListener('touchstart', this.handleDragStart, {
        passive: false
      ...
      anchors.forEach(function (dom) {
        dom.addEventListener('touchstart', _this.handleResizeDragStart, {
          passive: false
        });
      });
      ...
      document.addEventListener('touchmove', this.handleDragOver, {
        passive: false
      });
    } else {
      this.$viewport.addEventListener('mousedown', this.handleDragStart, {
        passive: false
      ...
      anchors.forEach(function (dom) {
        dom.addEventListener('mousedown', _this.handleResizeDragStart, {
          passive: false
        });
      });
      ...
      document.addEventListener('mousemove', this.handleDragOver, {
        passive: false
      });
    }
  };
  ...
  ImageClip.prototype.componentWillUnmount = function () {
    ...
    var anchors = [this.$leftTopAnchor, this.$leftCenterAnchor, this.$leftBottomAnchor, this.$topCenterAnchor, this.$bottomCenterAnchor, this.$rightBottomAnchor, this.$rightCenterAnchor, this.$rightTopAnchor];

    if (isTouchScreen) {
      this.$viewport.removeEventListener('touchstart', this.handleDragStart); // this.$viewport.removeEventListener('touchmove', this.handleDragOver);
      ...
      this.$viewport.removeEventListener('mousedown', this.handleDragStart); // this.$viewport.removeEventListener('mousemove', this.handleDragOver);
  ...
  ImageClip.prototype.componentWillReceiveProps = function (nextProps) {
    if (nextProps.getDataURLDelegator !== this.props.getDataURLDelegator && typeof nextProps.getDataURLDelegator === 'function') {
      nextProps.getDataURLDelegator(this.internalGetDataURL);
    }

    if (nextProps.src !== this.props.src || nextProps.ratio !== this.props.ratio || nextProps.canOverClip !== this.props.canOverClip) {
      ...
      return tslib_1.__generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 5:
            ...
            layoutInfo = calcContainerSize(this.props.ratio, this.props.canOverClip, this.containerWidth, this.image.naturalWidth, this.image.naturalHeight);
  ...
  Object.defineProperty(ImageClip.prototype, "viewportStyle", {
    get: function () {
      var _a = this.state.viewportPos,
          height = _a.height,
          width = _a.width,
          left = _a.left,
          top = _a.top;
      var _b = this.state.dragOffset,
          x = _b.x,
          y = _b.y;
      return {
        backgroundImage: "url(" + this.props.src + ")",
        ...
        height: height,
        width: width,
        backgroundPosition: this.state.backgroundPositionX - left - (this.state.isDragging ? x : 0) + "px" + (" " + (this.state.backgroundPositionY - top - (this.state.isDragging ? y : 0)) + "px"),
  ...
  Object.defineProperty(ImageClip.prototype, "min", {
    get: function () {
      var width = getDistance(this.props.minWidth || '37%', this.containerWidth, this.image.naturalWidth);
      var height = getDistance(this.props.minHeight || '37%', this.state.containerHeight, this.image.naturalHeight);

      if (this.props.ratio) {
        // 以高基准计算宽最小值
        var hBaseMinWidth = height * this.props.ratio; // 以宽基准计算高最小值
  ...
  ImageClip.prototype.render = function () {
    return React.createElement("div", {
      ...
      className: "clip-container-loading",
      style: {
        display: this.state.loading ? 'flex' : 'none'
      }
    }, "\u56FE\u7247\u52A0\u8F7D\u4E2D..."), React.createElement("div", {
      className: "clip-viewport" + (this.state.isResizing ? ' clip-resizeing' : ''),
      ref: this.getViewport,
      style: this.viewportStyle
    }, React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-lt",
      "data-type": "lefttop",
      ref: this.getLeftTopAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-lb",
      "data-type": "leftbottom",
      ref: this.getLeftBottomAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-rt",
      "data-type": "righttop",
      ref: this.getRightTopAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-rb",
      "data-type": "rightbottom",
      ref: this.getRightBottomAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-tm",
      "data-type": "topcenter",
      ref: this.getTopCenterAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-bm",
      "data-type": "bottomcenter",
      ref: this.getBottomCenterAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-lm",
      "data-type": "leftcenter",
      ref: this.getLeftCenterAnchor
    }), React.createElement("div", {
      style: {
        display: this.state.failed ? 'none' : 'block'
      },
      className: "clip-anchor clip-anchor-rm",
      "data-type": "rightcenter",
      ref: this.getRightCenterAnchor
    })));
  };
