...
export function clipImageByPosition(img, x, y, width, height, type, quality, backgroundColor, padLeft, padTop) {
  if (type === void 0) {
    type = 'jpeg';
  }
...
export function calcContainerSize(ratio, canOverClip, wrapperWidth, natureWidth, natureHeight) {
  var viewportX;
  var viewportY;
  var viewportWidth;
  var viewportHeight;
...
export function calcOffsetEdge(containerX, containerY, containerWidth, containerHeight, viewportX, viewportY, viewportWidth, viewportHeight, type, minWidth, minHeight, anchorType, ratio) {
  var minX;
  var minY;
  var maxX;
  var maxY;
  ...
  if (type === 'move') {
    ...
    var deltaX = void 0; // 锚点离边框的距离（放大方向）

    var deltaY = void 0; // 计算点击的锚点到边框的距离
...
export function calcScalePos(viewportX, viewportY, viewportWidth, viewportHeight, _deltaX, _deltaY, type, ratio) {
  var x = viewportX;
  var y = viewportY;
...
export function getDistance(l, m, naturalSize) {
  if (!l) {
    return 100;
  }
...
export function prefixClipInfo(info, containerWidth, containerHeight) {
  var x = info.x,
      y = info.y,
      width = info.width,
      height = info.height;
...
export function distanceOfPoints(x0, y0, x1, y1) {
  if (x1 === void 0) {
    x1 = 0;
  }
